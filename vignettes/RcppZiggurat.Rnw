...
\textsl{random} numbers are by definition deterministic. Here, we follow
standard convention and refer to such numbers as \textsl{pseudo-random} as
they can always be recreated given the identical seed value. We are not
...
combination of both excellent statistical properties as well as execution speed.
...
%% this setting influences page breaks somewhat -- with 0.525 we get two
%% plots onto one page
...
@
  \caption{Standard Test applied to Ziggurat generators \label{fig:StandardTest}}
  \medskip
    \begin{minipage}{0.75\linewidth}
...
@
  \caption{Normal Test applied to Ziggurat generators \label{fig:NormalTest}}
  \medskip
    \begin{minipage}{0.75\linewidth}
      \scriptsize Note: Code for the figure is included in the
      \pkg{RcppZiggurat} package, and the source code for this document.
...
the result discussed above. Kolmogorov-Smirnow rejects for the
\cite{Marsaglia+Tsang:2000} generator. However, we note that the Wilcoxon
test now has a lower $p$-value---though we would still not yet reject at
...
@
  \caption{$\chi^2$ Test applied to Ziggurat generators \label{fig:ChiSqTest}}
  \medskip
    \begin{minipage}{0.75\linewidth}
      \scriptsize Note: Code for the figure is included in the
      \pkg{RcppZiggurat} package, and the source code for this document.
...
in a parellel execution framework.
