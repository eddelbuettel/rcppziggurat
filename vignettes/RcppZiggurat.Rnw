...
\abstract{
  ...
  This note provides updated implementations of the Ziggurat generator
...
scientific computing and about as old the field itself. Most algorithms
...
Regarding terminology, all algorithms for generation \textsl{random} numbers
are by definition deterministic. Here, we follow standard convention and
refer to such numbers as \textsl{pseudo-random} as they can always be
recreated given the identical seed value, and are therefore fully
deterministic. We are not concerned in this note with \textsl{quasi-random}
numbers (also called low-discrepnancy sequences). We consider this topic to
be subset of pseudo-random numbers subject to distributional
constraints. Without lack of generality, we will henceforth drop the
qualifier \textsl{pseudo} when refering to random numbers.
...
\citep{Box+Muller:1958}. A useful recent survey is provided by
...
popular\footnote{Usage of a code search engine such as \url{code.ohloh.net}
  ...
  popularity of the Ziggurat algorithm as its name is a reasonably unique
  search term within the field of computing.} as it strikes a reasonably
between excellent statistical properties as well as execution speed.
...
implementation, and compares it to several other Open Source implementation
of the underlying algorithm by applying three different statistical.
...
functions. The helper function allow setting a seed, and deal with parameter
updated need in about 2.5\% of cases.  The actual core component---the
function to draw a random number distributed according to standard normal
...
\texttt{int} is still 32 bits).c

The modified version suggested by this paper overcomes both issues.
...
authors show via $\chi^2$-test that the generator has a short period of about
...
@
  \caption{Improved Ziggurat code by \cite{Leong+Zhang+Lee+Luk+Villasenor:2005}
    \label{fig:ZiggLZLLV}}
  \end{center}
\end{figure}
...
Mersenne Twister \citep{Matsumoto+Nishimura:1998}.

\subsection{Voss's implementation in GNU GSL}
...
of 256 steps which reduces the memory requirements at the possible expense of
precision. Second, he uses an exponential distribution with tail rejection
...
\begin{figure}[!tp]
  \begin{center}
<<R_normal_RNGs_Plot,echo=FALSE,fig=TRUE>>=
library(lattice)
print(bwplot(time/1e6 ~ expr, as.data.frame(rspeed),
             ylab="Time in msec", main="Time for 100 times 1e6 normal draws",
             panel=function(...,box.ratio) {
                 panel.violin(..., col="lightgray", varwidth=FALSE, box.ratio=box.ratio)
             }))
@
  \caption{R Normal RNG Generator Performance \label{fig:RNormalRNGs}}
  \end{center}
\end{figure}
...
chosen as the default in R. So R users could reap a speed benefit simply by
calling \code{RNGkind(,"Ahrens-Dieter")} or
\code{RNGkind(,"Kinderman-Ramage")}.

\subsection{Ziggurat Generators}
...
\begin{figure}[!tp]
  \begin{center}
<<Zigg_normal_RNGs_Plot,echo=FALSE,fig=TRUE>>=
library(lattice)
#res <- readRDS("~/git/rcppziggurat/vignettes/Zigspeed.rds")
print(bwplot(time/1e6 ~ expr, as.data.frame(zigspeed),
             ylab="Time in msec", main="Time for 100 times 1e6 normal draws",
             panel=function(...,box.ratio) {
                 panel.violin(..., col="lightgray", varwidth=FALSE, box.ratio=box.ratio)
             }))
@
  \caption{Ziggurant and R Normal RNG Generator Performance \label{fig:ZiggRNormalRNGs}}
  \end{center}
\end{figure}

All Ziggurat generators are significantly faster the the default generator in
R which uses a inversion method.  Among the Ziggurat generator, we notice
...
implementation which is seen as the fastest method.  A comparison to the
two original implementations (not shown) shows them to be of comparable
speed.  However, as the next section details, the initial implementation
should not be used. And as discussed below, the bare-bones implementation of
the initial proposals lack some safeguards suggested by modern software practice
such as encapsulation within a class, a namespace or both.
...
\item Repeats this $m$ times to create a set of $m$ sums of uniform RNG draws.
...
\item We now have $m$ uniformly distributed values. A standard test such as
  Kolmogorov-Smirnow or Wilcoxon can be used to test against departures from
  the uniform distribution.
\end{enumerate}

Here, we can apply this test for first converting out $N(0,1)$ distributed
values to an initial $U(0,1)$ distribution by using the inverse of the normal
distribution.
...
the expected diagonal clearly once we draw more than $4.2 \times 10^9$ number
(which is the limit of the representation of an unsigned four-byte nunber).
However, only the Kolmogorov-Smirnow test can formally reject; the Wilcoxon
test appear to lack sufficient power in this setting.
...
Results are qualitatively similar. Kolmogorov-Smirnow rejects for the
\cite{Marsaglia+Tsang:2000} generator. However, we note that the Wilcoxon test now
has a lower $p$-value though we would still not yet reject at conventional
test levels.
...
The original proposal by \cite{Marsaglia+Tsang:2000} fails as was shown by
\cite{Leong+Zhang+Lee+Luk+Villasenor:2005}. All other tests pass again.

\begin{figure}[!tp]
  \begin{center}
<<chisqtest, echo=FALSE, print=FALSE, fig=TRUE>>=
...
\cite{Leong+Zhang+Lee+Luk+Villasenor:2005} provided implementations in the C
language.  These implementations were provided in just a few lines, and used
idioms common to C programmers such as macros and global variables.

C++ programming style permits encapsulation in order to avoid possibles
collisions and side-effects. Moreover, by using a most amount of
...
Figure~\ref{fig:ZigguratBase} shows the virtual base class used here. We
provide two user-accessible functions to obtain a normal random deviate, and
to set the seed, respectively.

Each actual implementation can then encapsulate its state variable as private
variable inaccessible to other functions. Such a small core to each class
also makes it feasible to provide a Ziggurat generator in each thread in a
parellel execution framework.
...
#endif
@
  \caption{Ziggurat Base Class in package \pkg{RcppZiggurat}. \label{fig:ZigguratBase}}
  \end{center}
\end{figure}
...
In the \pkg{RcppZiggurat} package, Rcpp Attributes
\citep{CRAN:Rcpp:Attributes} are used to access instances of the
...
@
  \caption{R access to Ziggurat Class via Rcpp Attributes. \label{fig:RAccess}}
  \end{center}
\end{figure}
...
generators still use global variables---which are therefore each shielded in
their own namespace.  For example, for the \cite{Marsaglia+Tsang:2000}
generator, we use \code{Ziggurat::ZigurratMT}.  Next is the name of the
actual class---which in  the case of the reference implementation shown in
Figure~\ref{fig:RAccess} is once again \code{Ziggurat} leading to the triple
use of the term.

The remainder of Figure~\ref{fig:RAccess} shows how two functions
\code{znorm()} and \code{zsetseed()} are provided via the attribute
\code{[[Rcpp::Export]]} as described in the \citep{CRAN:Rcpp:Attributes}
vignette of the \pkg{Rcpp} package \citep{CRAN:Rcpp,Eddelbuettel:2013:Rcpp}.


\section{TODO}
\begin{itemize}
\item Possibly integrate into RDieHarder, or consider Big Crush from TestU01
...
\begin{itemize}
\item Updated Ziggurat: now providing portable code for 32 + 64 bit OSs
\item Clean C++ wrapper facilitates comparison \& testing
\item Reconfirmed Leong et al of MT failing at 32 bit limit
\item Applied standard random number test (for uniforms); confirming result
\item Suggested new test based directly on normal random deviates; also
  confirms MT issue
\item Tested + validated three other OS implementations
\item Timings favourable: our variant fastest across other Ziggurat
  implementations and the R generators
\end{itemize}
