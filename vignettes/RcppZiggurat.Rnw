...
Figure~\ref{fig:ZigguratBase} shows the virtual base class used here.
...
As shown in Figure~\ref{fig:ZigguratBase}, we provide two user-accessible
...
\citep{CRAN:Rcpp:Attributes} feature of the \pkg{Rcpp} C++/R integrations
package \citep{CRAN:Rcpp,Eddelbuettel:2013:Rcpp} are used to access instances of
...
class---which in the case of the reference implementation shown in
