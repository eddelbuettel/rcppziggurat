...
\code{[[Rcpp::Export]]} as described in the Rcpp Attributes vignette
\citep{CRAN:Rcpp:Attributes} of the \pkg{Rcpp} package
\citep{CRAN:Rcpp,Eddelbuettel:2013:Rcpp}.
...
but do not find a problem with any of the new implementations.

A key motivation for this work has been a desire to improve the speed of
(standard-normally distributed) random numbers for R.  We find Ziggurat to be
...
\code{stdint} header file for 32- and 64-bit use, as well as the need to
