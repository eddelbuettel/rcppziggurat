...
which generate stream of normally distributed (pseudo) random
numbers.\footnote{All algorithms for generation \textsl{random} number are by
  definition deterministic. We follow standard convention and refer to such
  numbers as \textsl{pseudo-random} as they can be recreated given the
  identical seed. We are not concerned in this not with \textsl{quasi-random}
  numbers (which are also called low-discrepnancy sequences) which we
  consider to be subset of pseudo-random number subject to distributional
  constraints. Without lack of generality, we will henceforth drop the
  qualifier \textsl{pseudo} when refering to random numbers.}
A well-known examples for such algorithms includes the method by Box and
Muller \citep{Box+Muller:1958}. A useful recent survey is provided by
...
TBD. Mention description and tests \cite{Yalta+Schreiber:2012}.
...
We see that the Box-Muller generator is by far the slowest. However, both
Kinderman-Ramage and Ahrens-Dieter are faster than the Inversion method
...
%% \begin{figure}[!htp]
%%   \begin{center}
%% <<Zigg_normal_RNGs_Tab,echo=FALSE>>=
%% library(microbenchmark)
%% res <- readRDS("~/git/rcppziggurat/vignettes/Zigspeed.rds")
%% print(summary(res, row.names=FALSE))
%% @
%%   \caption{Ziggurant and R Normal RNG Generator Performance Table\label{tab:ZiggRNormalRNGs}}
%%   \end{center}
%% \end{figure}
...
Marsaglia and Tsang generator. However, we note that the Wilcoxon test now
...
The original proposal by \citep{Marsaglia+Tsang:2000} fails as was shown by
...
TBD. Reference Figure~\ref{fig:ZigguratBase} and virtual base class.

Also mention suitable for parallel exectution as each thread will have its
own (smallish) generator.
...
Mention/show user-supplied hook using \code{RNGkind(,"user-supplied")}.
